...
import { startPolling, stopPolling, sendMessage, sendHTML, sendMessageWithButtons, notifyOutOfRange, isEnabled as telegramEnabled } from "./telegram.js";
...
Example: "AVOID: Entering NOTHING-SOL during 4h +70% pump — reversal risk is high. Top LPers hold 0.2h in this pool but we held 3.8h. Next time: match scalper cadence or skip pumping tokens."
      ...
      // Dynamic management interval based on position volatility
      try {
        const openPos = await getMyPositions().catch(() => null);
        const { getTrackedPosition } = await import('./state.js');
        const maxVol = (openPos?.positions || []).reduce((max, p) => {
          const tracked = getTrackedPosition(p.position);
          return Math.max(max, tracked?.volatility ?? 0);
        }, 0);
        const targetInterval = maxVol >= 5 ? 3 : maxVol >= 2 ? 5 : 10;
        if (config.schedule.managementIntervalMin !== targetInterval) {
          config.schedule.managementIntervalMin = targetInterval;
          log('cron', `Management interval adjusted to ${targetInterval}m (max volatility: ${maxVol})`);
          startCronJobs();
        }
      } catch { /* best-effort */ }
...
async function handleCallbackQuery(data) {
  if (data.startsWith("close:")) {
    ...
    if (_managementBusy || _screeningBusy || busy) {
      await sendMessage("Agent is busy — try again in a moment.").catch(() => {});
      return;
    }
    busy = true;
    try {
      await sendMessage("Closing position: " + pair + "...");
      const { content } = await agentLoop(`Close the position for ${pair}. Find the position address and close it. Report result.`, config.llm.maxSteps, [], "MANAGER", config.llm.managementModel);
  ...
  if (data.startsWith("claim:")) {
    ...
    busy = true;
    try {
      await sendMessage("Claiming fees for position " + position.slice(0, 8) + "...");
      const { content } = await agentLoop(`Claim fees for position address ${position}. Report result.`, config.llm.maxSteps, [], "MANAGER", config.llm.managementModel);
      await sendMessage(content);
    } catch (e) { await sendMessage("Error: " + e.message).catch(() => {}); }
    finally { busy = false; }
    return;
...
async function handleTelegramMessage(text) {
    ...
    if (cmd === "/status") {
      try {
        ...
        let msg = "WALLET\n";
        msg += "SOL: " + (portfolio.sol || 0).toFixed(4) + " ($" + (portfolio.sol_usd || 0).toFixed(2) + ")\n";
        msg += "USDC: $" + (portfolio.usdc || 0).toFixed(2) + "\n";
        msg += "Total: $" + (portfolio.total_usd || 0).toFixed(2) + "\n\n";
        msg += "POSITIONS (" + pos.length + " open)\n";
        pos.forEach((p, i) => {
          msg += (i+1) + ". " + p.pair + "\n";
          msg += "   Fees: $" + (p.unclaimed_fees_usd || 0).toFixed(2) + " | PnL: " + (p.pnl_pct || 0).toFixed(2) + "%\n";
          msg += "   " + (p.in_range ? "In Range" : "Out of Range") + " | Age: " + (p.age_minutes || 0) + "m\n";
        });
    ...
    if (cmd === "/balance") {
      try {
        ...
        const tkns = (portfolio.tokens || []).filter(t => t.symbol !== "SOL" && t.symbol !== "USDC" && t.usd > 0.01);
        let msg = "BALANCE\n";
        msg += "SOL: " + (portfolio.sol || 0).toFixed(4) + " ($" + (portfolio.sol_usd || 0).toFixed(2) + ")\n";
        msg += "USDC: $" + (portfolio.usdc || 0).toFixed(2) + "\n";
        if (tkns.length) msg += "Tokens: " + tkns.map(t => t.symbol + " ($" + (t.usd || 0).toFixed(2) + ")").join(", ") + "\n";
        msg += "Total: $" + (portfolio.total_usd || 0).toFixed(2);
        await sendMessage(msg);
      } catch (e) { await sendMessage("Error: " + e.message).catch(() => {}); }
      return;
    ...
    if (cmd === "/positions" || cmd === "/position") {
      try {
        const positions = await getMyPositions();
        const pos = positions.positions || [];
        if (pos.length === 0) { await sendMessage("No open positions."); return; }
        for (let i = 0; i < pos.length; i++) {
          const p = pos[i];
          const msg =
            (i+1) + ". " + p.pair + "\n" +
            "Pool: " + p.pool.slice(0, 8) + "...\n" +
            "PnL: " + (p.pnl_pct || 0).toFixed(2) + "% ($" + (p.pnl_usd || 0).toFixed(2) + ")\n" +
            "Fees: $" + (p.unclaimed_fees_usd || 0).toFixed(2) + "\n" +
            (p.in_range ? "✅ In Range" : "⚠️ Out of Range") + " | Age: " + (p.age_minutes || 0) + "m";
          const buttons = [[
            { text: "❌ Close", callback_data: "close:" + p.pair },
            { text: "💰 Claim Fee", callback_data: "claim:" + p.position },
          ]];
    ...
    if (cmd === "/pnl") {
      try {
        ...
        let msg = "PNL SUMMARY\n\n";
        msg += "Total positions: " + (perf.total_positions || 0) + "\n";
        msg += "Win rate: " + (perf.win_rate || 0).toFixed(1) + "%\n";
        msg += "Avg PnL: " + (perf.avg_pnl_pct || 0).toFixed(2) + "%\n";
        msg += "Total fees: $" + (perf.total_fees_usd || 0).toFixed(2) + "\n";
        msg += "Net PnL: $" + (perf.net_pnl_usd || 0).toFixed(2);
        await sendMessage(msg);
      } catch (e) { await sendMessage("Error: " + e.message).catch(() => {}); }
      return;
    }
    ...
    if (cmd === "/fees") {
      ...
      busy = true;
      try {
        await sendMessage("Claiming all unclaimed fees...");
        const { content } = await agentLoop("Check all positions and claim fees for any position with unclaimed fees > $1. Report results.", config.llm.maxSteps, [], "MANAGER", config.llm.managementModel);
        await sendMessage(content);
      } catch (e) { await sendMessage("Error: " + e.message).catch(() => {}); }
      finally { busy = false; }
      return;
    ...
    if (cmd === "/candidates") {
      ...
      busy = true;
      try {
        await sendMessage("Fetching top pool candidates...");
        const { content } = await agentLoop("Get top pool candidates using get_top_candidates and show key metrics for each. No deploy.", config.llm.maxSteps, [], "SCREENER", config.llm.screeningModel);
        await sendMessage(content);
      } catch (e) { await sendMessage("Error: " + e.message).catch(() => {}); }
      finally { busy = false; }
      return;
    ...
    if (cmd.startsWith("/blacklist ")) {
      ...
      const mint = parts[1];
      const reason = parts.slice(2).join(" ") || "Manually blacklisted by user";
      if (!mint || mint.length < 30) {
        await sendMessage("Usage: /blacklist <mint_address> [reason]");
        return;
      }
      try {
        const { addToBlacklist } = await import("./token-blacklist.js");
        const result = addToBlacklist({ mint, symbol: mint.slice(0, 8), reason });
        await sendMessage("Blacklisted: " + mint.slice(0, 8) + "...\nReason: " + reason);
      } catch (e) { await sendMessage("Error: " + e.message).catch(() => {}); }
      return;
    }
    ...
    if (cmd.startsWith("/close ")) {
      ...
      const pair = text.trim().split(/\s+/).slice(1).join(" ");
      busy = true;
      try {
        await sendMessage("Closing position: " + pair + "...");
        const { content } = await agentLoop(`Close the position for ${pair}. Find the position address and close it. Report result.`, config.llm.maxSteps, [], "MANAGER", config.llm.managementModel);
    ...
    if (cmd === "/thresholds") {
      try {
        ...
        let msg = "CURRENT THRESHOLDS\n\n";
        msg += "minFeeActiveTvlRatio: " + s.minFeeActiveTvlRatio + "\n";
        msg += "minTvl: $" + s.minTvl + "\n";
        msg += "maxTvl: $" + s.maxTvl + "\n";
        msg += "minVolume: $" + s.minVolume + "\n";
        msg += "minOrganic: " + s.minOrganic + "\n";
        msg += "minHolders: " + s.minHolders + "\n";
        msg += "minMcap: $" + s.minMcap + "\n";
        msg += "timeframe: " + s.timeframe + "\n";
        msg += "takeProfitFeePct: " + m.takeProfitFeePct + "%\n";
        msg += "outOfRangeWaitMinutes: " + m.outOfRangeWaitMinutes + "m";
        await sendMessage(msg);
      } catch (e) { await sendMessage("Error: " + e.message).catch(() => {}); }
      return;
    }
    ...
    if (cmd === "/evolve") {
      ...
      busy = true;
      try {
        await sendMessage("Evolving thresholds based on performance...");
        const { content } = await agentLoop("Analyze closed position performance and evolve screening thresholds.", config.llm.maxSteps, [], "GENERAL", config.llm.generalModel);
        await sendMessage(content);
      } catch (e) { await sendMessage("Error: " + e.message).catch(() => {}); }
      finally { busy = false; }
      return;
    ...
    if (cmd.startsWith("/learn")) {
      ...
      const pool = text.trim().split(/\s+/)[1] || null;
      busy = true;
      try {
        const goal = pool ? `Study top LPers from pool ${pool} and save lessons.` : "Study top LPers across all current candidate pools and save lessons.";
        await sendMessage("Learning from top LPers...");
        const { content } = await agentLoop(goal, config.llm.maxSteps, [], "GENERAL", config.llm.generalModel);
        await sendMessage(content);
      } catch (e) { await sendMessage("Error: " + e.message).catch(() => {}); }
      finally { busy = false; }
      return;
    ...
    // ── /stop ────────────────────────────────────────────────
    if (cmd === "/stop") {
      await sendMessage("Shutting down agent...");
    ...
    if (cmd === "/help" || cmd === "/start") {
      await sendMessage(
        "AVAILABLE COMMANDS\n\n" +
        "/status — wallet + positions summary\n" +
        "/balance — wallet balance detail\n" +
        "/positions — all open positions detail\n" +
        "/pnl — performance summary\n" +
        "/candidates — top pool candidates\n" +
        "/fees — claim all unclaimed fees\n" +
        "/close <pair> — close position (e.g. /close CHIBI-SOL)\n" +
        "/blacklist <mint> [reason] — blacklist a token\n" +
        "/thresholds — current screening thresholds\n" +
        "/learn [pool] — study top LPers\n" +
        "/evolve — evolve thresholds from performance\n" +
        "/briefing — morning briefing\n" +
        "/stop — shutdown bot\n\n" +
        "Or just chat freely for AI assistance."
      );
      return;
    }
