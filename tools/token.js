...
export async function getTokenNarrative({ mint }) {
  const res = await fetch(`${DATAPI_BASE}/chaininsight/narrative/${mint}`, { signal: AbortSignal.timeout(10_000) });
...
export async function getTokenHolders({ mint, limit = 20 }) {
  ...
  const [holdersRes, tokenRes, gmgnInfo] = await Promise.all([
    fetch(`${DATAPI_BASE}/holders/${mint}?limit=100`, { signal: AbortSignal.timeout(10_000) }),
    fetch(`${DATAPI_BASE}/assets/search?query=${mint}`, { signal: AbortSignal.timeout(10_000) }),
  ...
  if (smartWallets.length > 0) {
    ...
    await Promise.all(matchedHolders.map(async (h) => {
      ...
      let pnl = null;
      try {
        const pnlRes = await fetch(`${DATAPI_BASE}/pnl-positions?address=${h.addr}&assetId=${mint}`, { signal: AbortSignal.timeout(8_000) });
        if (pnlRes.ok) {
