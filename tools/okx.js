...
export async function getRiskFlags(tokenAddress, chainId = CHAIN_SOLANA) {
  // /priapi/ path may also require auth on some OKX regions — skip gracefully if no key
...
export async function getAdvancedInfo(tokenAddress, chainIndex = CHAIN_SOLANA) {
  // /api/v6/ endpoints require valid API key — skip gracefully if not configured
  if (!hasAuth()) return null;
...
export async function getFullTokenAnalysis(tokenAddress, chainIndex = CHAIN_SOLANA) {
  if (!hasAuth()) {
    // No API key configured — only cluster data available (public endpoint)
    const clusters = await getClusterList(tokenAddress, chainIndex).catch(() => []);
    return { advanced: null, clusters, price: null };
  }
