...
import { NuggetShelf, promoteFacts } from "./nuggets.js";
...
export function initMemory() {
  // Define maxFacts per nugget — passed to loadAll so existing files
  // are loaded with the correct capacity (not silently capped at default 100)
  const FACT_LIMITS = {
    pools:      150,
    strategies: 80,
    lessons:    100,
  ...
  shelf.loadAll(FACT_LIMITS);

  // Ensure core nuggets exist (creates if not present on disk)
  shelf.getOrCreate("pools",      { maxFacts: FACT_LIMITS.pools });
  shelf.getOrCreate("strategies", { maxFacts: FACT_LIMITS.strategies });
  shelf.getOrCreate("lessons",    { maxFacts: FACT_LIMITS.lessons });
  shelf.getOrCreate("patterns",   { maxFacts: FACT_LIMITS.patterns });
...
export function rememberPositionSnapshot(position) {
  ...
  // Pool behavior snapshot — use configured display mode
  ...
  const useSol = config.management.solMode === true;
  const fees = useSol && position.unclaimed_fees_sol != null
    ? `${position.unclaimed_fees_sol} SOL`
    : (position.unclaimed_fees_usd != null ? `$${position.unclaimed_fees_usd}` : "?");
